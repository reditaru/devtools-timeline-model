...
class SandboxedModel {
  ...
  init(events) {
    // build empty models. (devtools) tracing model & performance model
    //   from Timeline.TimelinePanel() constructor
    const tracingModelBackingStorage = new Bindings.TempFileBackingStorage();
  ...
  topDownGroupBy(grouping, startTime = 0, endTime = Infinity) {
    ...
    const topDownGrouped = new TimelineModel.TimelineProfileTree.TopDownRootNode(this._performanceModel._timelineModel.tracks().find(track => track.type === TimelineModel.TimelineModel.TrackType.MainThread).syncEvents(),
      filters, startTime, endTime, false, groupingAggregator);
  ...
  bottomUpGroupBy(grouping, startTime = 0, endTime = Infinity) {
    ...
    const groupingAggregator = this._createGroupingFunction(Timeline.AggregatedTimelineTreeView.GroupBy[grouping]);
    const bottomUpGrouped = new TimelineModel.TimelineProfileTree.BottomUpRootNode(this._performanceModel._timelineModel.tracks().find(track => track.type === TimelineModel.TimelineModel.TrackType.MainThread).syncEvents(),
      filters, startTime, endTime, false, groupingAggregator);
  ...
  filmStripModel() {
    if (!this._performanceModel.filmStripModel) {
      this._filmStripModel = new SDK.filmStripModel(this._performanceModel._tracingModel);
    }
    return this._filmStripModel;
  }

  interactionModel() {
    return this._performanceModel._irModel;
  }
}
